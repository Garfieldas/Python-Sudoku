...
def win():
    ...
    record = x
    mycursor.execute("UPDATE Sudoku set level5 = %s", (record,))
    db.commit()
    
        
    if (record == 45):
        messagebox.showinfo("Information", "Sveikiname gavus sviestą bei"+ str(record) +' tasku!')
        title()
        
    else:
        messagebox.showwarning("Warning", "Dėja sviesto nebus "+ str(record) + ' ,nes tiek tasku')
